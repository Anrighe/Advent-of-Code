...
def shortestPath(filename):
    """Populates a graph based on the terrain input and finds the shortest path from any S or a to E"""
    heightGridChar = []
    heightGridNum = []
    rowCount = 0
    colCount = 0
    ...
    with open(filename, "r") as file:
        ...
        for row in range(rowCount):
            for col in range(colCount):
                ...
                if heightGridChar[row][col] == 'a' or element == 'S':
                    try:
                        pathLen = find_path(graph, startingPosition, finalPosition)[3]
...
if __name__ == '__main__':
    ...
    print(f'\nThe goal has been reached from the closest position (in terms of steps) with altitude 0 in: {stepCounter} steps')
