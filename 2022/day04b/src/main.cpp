...
/**
 * @brief returns true if the first interval specified by [firstValueFirstElf, secondValueFirstElf] overlaps  
 * the second interval [firstValueSecondElf, secondValueSecondElf]. Endpoints are included in the check
 * 
 ...
 */
bool isAssignmentOverlapping(int firstValueFirstElf, int secondValueFirstElf, int firstValueSecondElf, int secondValueSecondElf)
{
    if ((firstValueFirstElf >= firstValueSecondElf && firstValueFirstElf <= secondValueSecondElf) || (secondValueFirstElf >= firstValueSecondElf && secondValueFirstElf <= secondValueSecondElf))
        return true;
    else
        return false;
}
    ...
    try
    {
        ...
        while (f.is_open() == true && f.eof() == false)
        {
            if (f.eof() == false)
            {
                ...
                if (isAssignmentOverlapping(firstValueFirstElf, secondValueFirstElf, firstValueSecondElf, secondValueSecondElf) == true)
                    containedCount++;
                else
                {
                    if (isAssignmentOverlapping(firstValueSecondElf, secondValueSecondElf, firstValueFirstElf, secondValueFirstElf) == true)
                        containedCount++;
                }
            }
        } 
