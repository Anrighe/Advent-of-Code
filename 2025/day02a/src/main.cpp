    ...
    public:
        ...
        long get_invalid_indexes_sum() const {
            ...
            for (long current_value = lower_bound; current_value <= upper_bound; ++current_value) {
                std::string current_value_string = std::to_string(current_value);

                if (current_value_string.length() % 2 == 0) { // If the string has a even number of digits
                    // Split the string in the middle so both strings are of the same length
                    std::pair<std::string, std::string> split_pair = split_string_middle(current_value_string);
                    
                    // If the two strings are the same they are invalid
                    if (split_pair.first == split_pair.second) { 
