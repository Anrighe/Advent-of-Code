...
/**
 * Reads equations from a file, evaluates which can be solved using addition, multiplication, 
 * and concatenation, and calculates the total of all valid left-hand side values.
 */
public class Main {
    ...
    public static AbstractSyntaxTree buildAST(List<Long> rhsValues) {
        ...
        for (int valuePosition = 1; valuePosition < rhsValues.size(); ++valuePosition) {
            ...
            for (Node leaf : ast.getAllLeafs()) {
                ...
                leaf.setNodeToConcatenate(new Node(value));
            }
        }
        return ast;
    }
    ...
    /**
     * Checks if an equation can be solved by applying addition, multiplication, 
     * and concatenation between right-hand side values.
     * (Checks all the possible permutations of allowed operators).
     *
