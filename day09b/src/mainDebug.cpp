        ...
        while (f.is_open() == true && f.eof() == false)
        {
            if (f.eof() == false)
            {
                ...
                for(int i = 0; i < distance; i++) // one step at a time
                {
                    ...
                    std::cout<<"moved rope [0].x to: "<<rope[0].x<<std::endl;
                    std::cout<<"moved rope [0].y to: "<<rope[0].y<<std::endl;
                    ...
                    for (int j = 1; j < 10; j++)
                    {
                            ...
                            if (areKnotsClose(rope[j-1], rope[j]) == false) // if they are NOT touching it needs to move tail
                            {
                                ...
                                if (isHeadInTailCrossRange(rope[j-1], rope[j]) == true)
                                {
                                    if (rope[j-1].x == rope[j].x && rope[j].y < rope[j-1].y) // need to move tail up
                                    {
                                        move(rope[j], 'U', 1);
                                        movedDiag = false;
                                        std::cout<<"moved rope["<<j<<"].x to: "<<rope[j].x<<std::endl;
                                        std::cout<<"moved rope["<<j<<"].y to: "<<rope[j].y<<std::endl;
                                    }
                                    else
                                    {
                                ...
                                {   //     -3          -3          -1            0
                                    ...
                                    else
                                    {
                                        ...
                                        std::cout<<"moved rope["<<j<<"].x += ("<<rope[diagPos].x<<" - "<<rope[diagPos].xPrev<<")"<<std::endl;
                                        std::cout<<"moved rope["<<j<<"].y += ("<<rope[diagPos].y<<" - "<<rope[diagPos].yPrev<<")"<<std::endl;
                        ...
                        {
                            //std::cin>>a;
                            std::cout<<"ROPE POSITION:"<<std::endl;
                            for (int k = 0; k < 10; k++)
                            {
                                std::cout<<"rope["<<k<<"]: ("<<rope[k].x<<", "<<rope[k].y<<")"<<std::endl;
                    ...
                    std::cout<<"PAIR: "<<rope[9].x<<", "<<rope[9].y<<std::endl;
