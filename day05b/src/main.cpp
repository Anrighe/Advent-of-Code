...
/**
 * @brief moves moveCount stacks from stack1 EXACTLY AS THEY ARE to stack2
 * 
 * @param stacks 
 * @param moveCount 
 * @param stack1 
...
{
    char * buffer = new char[moveCount];
    for (int i = 0; i < moveCount; i++)
    {
        buffer[i] = stacks[stack1-1].top();
        stacks[stack1-1].pop();
    }
    for (int i = moveCount-1; i >= 0; i--)
        stacks[stack2-1].push(buffer[i]);
